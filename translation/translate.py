...
def grounded_action(
    ...
    return (
        action_to_z3(action.instantiate(arguments), t)
    )
...
def add_transition_function(
    ...
    for pred, [pos_acts, neg_acts] in inverse_effect_list(
        domain, problem
    ).items():
...
def main():
    ...
    while True:
        horizon += 1
        s.pop()  # Pop goal
        action_maps.append(add_transition_function(
    ...
    # Extract plan
    mod = s.model()
    plan: list[pddl.core.Action | None] = [None for _ in range(horizon)]
    for t in range(horizon):
        for v, a in action_maps[t].items():
            if mod.get_interp(v):
                ...
                continue
    for a in plan:
        print(f"({pretty_action(a)})" if a is not None else "()")
